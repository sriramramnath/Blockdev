...
const BLOCK_PATTERNS = [
    // Say/think patterns - MUST BE FIRST to avoid conflicts with repeat patterns
    {
        patterns: [
            /`say\s+\[([^\]]+)\]\s+for\s+(\d+(?:\.\d+)?)\s*seconds?`/i,  // `say [message] for 2 seconds`
    ...
    {
        patterns: [
            /`think\s+\[([^\]]+)\]\s+for\s+(\d+(?:\.\d+)?)\s*seconds?`/i,  // `think [message] for 3 seconds`
    ...
    {
        patterns: [
            /`move\s+(\d+)\s*steps?`/i,  // `move 10 steps`
        ...
        generate: (match, text) => {
            const steps = (match[1] || match[2] || match[3] || '10').trim();
            return {
                id: 'motion_movesteps',
                type: BLOCK_TYPES.COMMAND,
                label: 'move %1 steps',
    ...
    {
        patterns: [
            /`wait\s+(\d+(?:\.\d+)?)\s*seconds?`/i,  // `wait 1 seconds`
            /\b(wait|pause|delay)\s+(\d+(?:\.\d+)?)\s*(seconds?|secs?)?\b/i,
            /\bstop\s+for\s+(\d+(?:\.\d+)?)\b/i
        ],
        generate: (match, text) => {
            const seconds = (match[1] || match[2] || '1').trim();
            return {
                id: 'control_wait',
                type: BLOCK_TYPES.COMMAND,
...
function handleAISuggestion(text) {
    ...
    // Add "when flag clicked" as the first block if not already present
    blocks.push({
        id: 'event_whenflagclicked',
        type: 'hat',
        label: 'when ⚑ clicked',
        category: 'events',
        inputs: [],
        tooltip: 'Start the script when the green flag is clicked'
    });
    
    // Extract backtick-enclosed commands first (more specific)
    const backtickPattern = /`([^`]+)`/g;
    const backtickMatches = [...text.matchAll(backtickPattern)];
    
    console.log('Found backtick commands:', backtickMatches.map(m => m[1]));
    
    for (const match of backtickMatches) {
        const command = match[1].trim().toLowerCase();
        console.log('Processing command:', command);
        
        // Parse each specific command type
        if (command.includes('say') && command.includes('for') && command.includes('seconds')) {
            const sayMatch = command.match(/say\s+\[([^\]]+)\]\s+for\s+(\d+)\s+seconds/);
            if (sayMatch) {
                blocks.push({
                    id: 'looks_sayforsecs',
                    type: 'command',
        ...
        } else if (command.includes('think') && command.includes('for') && command.includes('seconds')) {
            const thinkMatch = command.match(/think\s+\[([^\]]+)\]\s+for\s+(\d+)\s+seconds/);
            if (thinkMatch) {
                blocks.push({
                    id: 'looks_thinkforsecs',
                    type: 'command',
                    ...
                    defaultValues: {
                        MESSAGE: thinkMatch[1].trim(),
                        SECS: thinkMatch[2]
                    },
                    tooltip: 'Display a thought bubble with the message for the specified time'
                });
            }
        } else if (command.includes('move') && command.includes('steps')) {
            const moveMatch = command.match(/move\s+(\d+)\s+steps/);
            if (moveMatch) {
                blocks.push({
                    id: 'motion_movesteps',
                    type: 'command',
                    ...
                    defaultValues: { STEPS: moveMatch[1] },
                    tooltip: 'Move the sprite forward by the specified number of steps'
                });
            }
        } else if (command.includes('wait') && command.includes('seconds')) {
            const waitMatch = command.match(/wait\s+(\d+(?:\.\d+)?)\s+seconds/);
            if (waitMatch) {
                blocks.push({
                    id: 'control_wait',
                    type: 'command',
                    ...
                    defaultValues: { DURATION: waitMatch[1] },
                    tooltip: 'Wait for the specified number of seconds'
                });
            }
        }
